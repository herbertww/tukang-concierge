...
- **Canonical logo:** `tukang-site/logo.svg` — the **user-provided** logo (do NOT redesign or substitute): white L-shaped mark (two rounded rects) + orange `#E8833A` square accent, wordmark `tukang.app` in **Space Grotesk 700** with an orange dot. Inlined directly into the nav (`.brand`) and footer (`.foot-brand`) rather than referenced via `<img>`; in the inline copies the `#FFFFFF` fills are swapped to `currentColor` so the logo stays legible on the light theme (the saved `.svg` keeps the original white fills). The saved `.svg` file itself isn't fetched by the page — it's the source-of-truth asset for regenerating other renders (OG image, favicon). Space Grotesk 700 is loaded via the Google Fonts `<link>` (replaced the old Schibsted Grotesk load).
